...
<<<<<<< HEAD
      ...
      if (batch %% 10 == 0) {
        ...
        write_rds(current_model, file = snap_path)
        prev_snap <- glue("{CHECKPOINT_DIR}/{sp_name}_batch{batch - 10}_snap.rds")
        if (file.exists(prev_snap)) file.remove(prev_snap)
        cat(glue("  Snapshot saved: {snap_path}\n"))
      }
      # --------------------
