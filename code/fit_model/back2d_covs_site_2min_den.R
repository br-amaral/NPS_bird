...
get_script_name <- function() {
  ...
  if (length(script_path) == 0) {
    ...
    script_path <- here::here("back2d_covs_site_2min_den.R")
  } else {
...
script_name <- 'back2d_covs_site_2min_den.R'
cat("\n", "\n", "\n", 
...
write_rds(jags.data, file = glue("data/ana_file/{date_out}_data_{sps_name}_{park_name}_site_den.rds"))
...
model_file <- "models/mod_1_vector1spsparks_simple_3covs_a0s_site_den.txt"
mod_name <- glue("data/ana_file/{date_out}_mod_{sps_name}_{park_name}_site_den.txt") %>% as.character()
...
file_name <- glue("{date_out}_{fil_nam}_{park_name}_{niterations}its_2min_site_den")
