...
for_cov <- forestNETN::joinStandData(park = "all") %>%
          as_tibble() %>% 
                 rename(pct_cro = Pct_Crown_Closure,,
                        pct_low = Pct_Understory_Low, 
                        pct_mid = Pct_Understory_Mid,
                        pct_hig = Pct_Understory_High,
                        pct_wat = Pct_Water) %>% 
                 arrange(Plot_Name) %>% 
                 select(Plot_Name, ParkUnit, xCoordinate, yCoordinate,
                        pct_cro, pct_low, pct_mid, pct_hig, SampleYear) %>% 
                distinct()
 
cwd_cov <- joinCWDData(park = 'all') %>% # coarse wood debris
                as_tibble() %>% 
                rename(CWD_vol_mean = CWD_Vol) %>%   #!TODO: any calculations like for BA?
                arrange(Plot_Name) %>% 
                select(Plot_Name, ParkUnit, CWD_vol_mean, SampleYear) %>%  
                distinct()
...
tre_cov <- joinTreeData(park = 'all', status = "live") %>% 
                ...
                ungroup() %>% 
                arrange(Plot_Name) %>% 
                select(Plot_Name, ParkUnit, BA_m2ha, treeden_ha, SampleYear) %>% 
                distinct()
...
div_cov <- joinTreeData(park = 'all', status = "live") %>% 
                ...
                ungroup() %>%
                arrange(Plot_Name) %>% 
                select(Plot_Name, ParkUnit, tree_rich, SampleYear) %>% 
                distinct()
...
shr_cov <- joinMicroShrubData(park = 'all') %>% 
              filter(#InvasiveNETN == F,
                     Shrub == 1) %>%   #!TODO:
              group_by(Plot_Name, SampleYear) %>% 
              mutate(pct_shr = sum(shrub_avg_cov, na.rm = T)) %>% 
              ungroup() %>% 
              select(Plot_Name, ParkUnit, pct_shr, SampleYear) %>% 
              arrange(Plot_Name) %>% 
              distinct() %>% 
              as_tibble()
...
stg_cov <- sumStrStage(park = "all") %>% 
              as_tibble() %>% 
              rename(pct_pol = pctBA_pole,
                     pct_mat = pctBA_mature,
                     pct_lar = pctBA_large) %>% 
              ungroup() %>% 
              select(Plot_Name, ParkUnit, pct_pol, pct_mat, pct_lar, SampleYear) %>% 
              distinct()  %>% 
              arrange(Plot_Name)

#? merge all plots data and connect plot data with bird data - neighbours
forest_covs <- left_join(for_cov, cwd_cov, by = c("Plot_Name", "ParkUnit", "SampleYear")) %>% 
                     left_join(., tre_cov, by = c("Plot_Name", "ParkUnit", "SampleYear")) %>% 
                     left_join(., div_cov, by = c("Plot_Name", "ParkUnit", "SampleYear")) %>% 
                     left_join(., shr_cov, by = c("Plot_Name", "ParkUnit", "SampleYear")) %>% 
                     left_join(., stg_cov, by = c("Plot_Name", "ParkUnit", "SampleYear"))  %>% 
                     filter(ParkUnit %!in% c("ACAD", "ELRO", "SAIR"))  %>%
                     rename(for_sit = Plot_Name)
...
ifelse(is.na(forest_covs$pct_shr), forest_covs$pct_shr <- 0, forest_covs$pct_shr <- forest_covs$pct_shr)
site_data_full <- forest_covs %>%
  select(-for_sit, -xCoordinate, -yCoordinate , -pct_shr , #-sta_str, -stg
         # -siteSTA,  -bird_sit    
         ) %>%
  filter(complete.cases(.))
