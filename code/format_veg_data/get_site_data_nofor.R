...
#? ------------------------------   get_site_data.R   ------------------------------
#? *********************************************************************************
#! Code to get site data from kate's dataset - connect the forest plot
#! information to the bird plot locations
#
#! Input ----------------------------------------------
...
for (ii in 1:nrow(bird_sit_coord2)) {
    ...
    par(mfrow = c(1,1))
    plot(for_sit_coord2$lonutm, for_sit_coord2$latutm, 
            col = "lightgreen", pch = "*")
    points(bird_sit_coord2$lonutm, bird_sit_coord2$latutm, 
            col = "gray", pch = "*")
    points(bird_sit_coord2$lonutm[ii], bird_sit_coord2$latutm[ii], 
            col = "black", pch = 19)
    ...
    for(jj in 1:nrow(for_sit_coord4)) {
        ...
        distances2 <- cbind(as.numeric(distances), 
                            for_sit_coord4$for_sit[jj]) %>% 
            as_tibble() %>% 
            rename(dist = V1, 
                for_sit = V2) %>% 
