...
park_list <- pk <- c("ACAD", "MABI", "MIMA", "MORR", "SAGA", "WEFA")
...
# perfor_p <- local_cors_p$perfor_m
basare_p <- local_cors_p$basare_m
regen_p <- local_cors_p$regen_m

local_cors <- local_cors %>% 
  mutate(site = as.numeric(sub('.', '', site)))

basare <- local_cors %>% 
  select(park,site,basare) %>% 
  pivot_wider(names_from = site, values_from = basare) %>% 
  select(-park) %>% 
  gtools::na.replace(0)

regen <- local_cors %>% 
  select(park,site,regen) %>% 
  pivot_wider(names_from = site, values_from = regen) %>% 
  select(-park) %>% 
  gtools::na.replace(0)

perfor <- local_cors %>% 
  select(park,site,perfor) %>% 
  pivot_wider(names_from = site, values_from = perfor) %>% 
  select(-park) %>% 
  gtools::na.replace(0)

#big_vars_fim$CHNG_TPA[is.nan(big_vars_fim$CHNG_TPA)]<-NA
#big_vars_fim$DIA_GROW[is.nan(big_vars_fim$DIA_GROW)]<-NA

# 
# big_vars_fim_m <- 
#   big_vars_fim %>% 
#     group_by(park) %>% 
#     summarise(CHNG_TPA_l = mean(!is.na(CHNG_TPA)),
#               BIO_ACRE_l = mean(BIO_ACRE, na.rm = T),
#               H_g_l = mean(H_g, na.rm = T),
#               Eh_a_l = mean(Eh_a, na.rm = T),
#               S_a_l = mean(S_a, na.rm = T),
#               BAA_l = mean(BAA, na.rm = T),
#               DIA_GROW_l = mean(!is.na(DIA_GROW)))
# 
# CHNG_TPA_l <- big_vars_fim_m$CHNG_TPA_l
# BIO_ACRE_l <- big_vars_fim_m$BIO_ACRE_l
# H_g_l <- big_vars_fim_m$H_g_l
# Eh_a_l <- big_vars_fim_m$Eh_a_l
# S_a_l <- big_vars_fim_m$S_a_l
# BAA_l <- big_vars_fim_m$BAA_l
# DIA_GROW_l <- big_vars_fim_m$DIA_GROW_l
