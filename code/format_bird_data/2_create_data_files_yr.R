...
#! Import data -----------------------------------------
## file paths
PATH_COVS_COUN <- "data/out/coun_covs_yr.rds"
PATH_COVS_PARK <- "data/out/park_covs_yr.rds"
PATH_COVS_SITE <- glue("data/out/site_covs_nei_grp_{radi_dist}m_yr.rds")
PATH_DIV_SITE_COVS <- "data/out/site_div_yr.rds"
PATH_DIV_PARK_COVS <- "data/out/park_div_yr.rds"
...
# added year here!!!
site_key <- y_dat6 %>% 
    select(Point_Name, site_n, park, Year) %>% 
    distinct()
...
site_covs <- read_rds(PATH_COVS_SITE) %>% 
                rename(Point_Name = bird_sit,
                       park = ParkUnit) %>%
                left_join(., site_key, by =  c("park", "Point_Name","Year")) %>% 
                select(-c(siteSTAYR, siteSAPdenYR, park))

X1 <- left_join(X, site_covs, by = c("Point_Name","site_n", "Year"))
dim(X1)
X1 %>% select(Point_Name,siteDENYR) %>% distinct() %>% arrange(siteDENYR) #%>% view()
...
X2 <- left_join(X1, park_covs, by = c("park", "Year"))
dim(X2)
...
X3 <- left_join(X2, coun_covs, by = c("park", "Year"))
dim(X3)
...
div_covs_site <- read_rds(PATH_DIV_SITE_COVS) %>% 
                    rename(siteH_g = S,
                           siteEh_g = J,
                           Year = SampleYear)
X4 <- left_join(X3, div_covs_site, by = c("Point_Name", "Year"))
dim(X4)
nrow(X4) == nrow(X3)
...
X5 <- left_join(X4, div_covs_park, by = c("park", "Year"))
dim(X5)
nrow(X5) == nrow(X4)
...
##### write files  ------
write_rds(y_dat8, file = "data/y_dat8_yr.rds")
write_rds(X7, file = "data/X_yr.rds")
