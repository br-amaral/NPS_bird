...
p <- which(pk == yf$Admin_Unit_Code[1], arr.ind = T)
...
# clustering index
x2s1 <- arr_clus_site1[p,1,1:nsite_pk[p]] %>% as.vector()
x2s2 <- arr_clus_site1[p,2,1:nsite_pk[p]] %>% as.vector()
x2s3 <- arr_clus_site1[p,3,1:nsite_pk[p]] %>% as.vector()
x2s4 <- arr_clus_site1[p,4,1:nsite_pk[p]] %>% as.vector()
x2s5 <- arr_clus_site1[p,5,1:nsite_pk[p]] %>% as.vector()
...
# number of core areas
x3s1 <- arr_core_site1[p,1,1:nsite_pk[p]] %>% as.vector()
x3s2 <- arr_core_site1[p,2,1:nsite_pk[p]] %>% as.vector()
x3s3 <- arr_core_site1[p,3,1:nsite_pk[p]] %>% as.vector()
x3s4 <- arr_core_site1[p,4,1:nsite_pk[p]] %>% as.vector()
x3s5 <- arr_core_site1[p,5,1:nsite_pk[p]] %>% as.vector()
...
sink("models/mod_occ_multisps_covs.txt")	
cat("	
model {	
...
str(jags.data <- list(y = yyy3,                    # bird detection array
                      ...
                      # npkM = dim(yyy3)[1])	     # number of years
...
jags_model <- rjags::jags.model(
  file = "models/mod_occ_multisps_covs.txt",
  data = jags.data,
  inits = inits,
  n.chains = nchains,
...
write_rds(samples_jags, 
          file = glue("data/model_res/samples_jags3_multisps_covs{yf2$Admin_Unit_Code[p]}_{niterations}its_run{length(list.files(path = file.path(getwd(),'/data/out'),pattern = glue('samples_jags'), full.names = FALSE)) + 1}.rds"))
