...
mod_name <- glue("models/{sps_loop}_step2_model{mod_nam_par}_scales{mod_nam_sca}_INT{date_step1}.txt") %>% 
              as.character()
...
if('beta1' %in% pars_mod$cov_name) {
    covs_mod_code <- glue("{covs_mod_code} \n
      ...
      beta_int[{beta_numb}] * X1[b, {pull(pars_mod[which(pars_mod$cov_name == 'beta1'),3])}] * Xp[b] + \n ")
...
if('beta2' %in% pars_mod$cov_name) {
    covs_mod_code <- glue("{covs_mod_code} \n
      ...
      beta_int[{beta_numb}] * X2[b, {pull(pars_mod[which(pars_mod$cov_name == 'beta2'),3])}] * Xp[b] + \n ")
    beta_numb <- beta_numb + 1
}     
...
if('beta3' %in% pars_mod$cov_name) {
    covs_mod_code <- glue("{covs_mod_code} \n
      ...
      beta_int[{beta_numb}] * X3[b, {pull(pars_mod[which(pars_mod$cov_name == 'beta3'),3])}] * Xp[b] + \n ")
    beta_numb <- beta_numb + 1
}     
...
if('beta4' %in% pars_mod$cov_name) {
    covs_mod_code <- glue("{covs_mod_code} \n
      ...
      beta_int[{beta_numb}] * X4[b, {pull(pars_mod[which(pars_mod$cov_name == 'beta4'),3])}] * Xp[b] + \n ")
    beta_numb <- beta_numb + 1
}   
...
if('beta5' %in% pars_mod$cov_name) {
    covs_mod_code <- glue("{covs_mod_code} \n
      ...
      beta_int[{beta_numb}] * X5[b, {pull(pars_mod[which(pars_mod$cov_name == 'beta5'),3])}] * Xp[b] + \n ")
    beta_numb <- beta_numb + 1
}  
...
# Find the line containing certain characters and insert interaction priors
target_pattern <- "tau.alpha0 ~ dgamma\\(0\\.1,0\\.1\\)"
target_line <- grep(target_pattern, mod_content1)
cat("Pattern found at line:", target_line, "\n")

# Insert interaction priors after the target line
if(length(target_line) > 0) {
  insert_pos <- target_line[1]
  mod_content1_modified <- c(mod_content1[1:insert_pos], 
                             "",
                             "  # Interaction term priors",
                             paste0("  ", beta_int_prior),
                             "",
                             mod_content1[(insert_pos + 1):length(mod_content1)])
} else {
  cat("Warning: Pattern not found, using original mod_content1\n")
  mod_content1_modified <- mod_content1
}

# Combine the content into a single string
mod_string <- paste(
  paste(mod_content1_modified, collapse = "\n"),
  covs_mod_code,
