...
sink("models/mod_occ_BLISS_multisps.txt")	
cat("	
model {	
...
jags_model <- rjags::jags.model(
  file = "models/mod_occ_BLISS_multisps.txt",
  data = jags.data,
  inits = inits,
  n.chains = nchains,
...
write_rds(jags_model, 
          file = glue("data/model_res/jags_model2_BLISS_multisps{yf2$Admin_Unit_Code[p]}_{niterations}its_run{length(list.files(path = file.path(getwd(),'/data/out'),pattern = glue('samples_jags'), full.names = FALSE)) + 1}.rds"))
...
write_rds(samples_jags, 
          file = glue("data/model_res/samples_jags3_BLISS_multisps{yf2$Admin_Unit_Code[p]}_{niterations}its_run{length(list.files(path = file.path(getwd(),'/data/out'),pattern = glue('samples_jags'), full.names = FALSE)) + 1}.rds"))



cat("\n\n\n DONE occupancy_BLISS_multisps.R \n\n\n\n")
