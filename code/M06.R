...
y1 %>% dplyr::select(Admin_Unit_Code) %>% table() %>% sort()
  ...
  # Likelihood	
    for(j in 1:nsiteM){       # site j	
      for(t in 1:nyrM){       # year t	
        ...
        logit(psi[j,t]) <- beta0 + 	
                             beta1 * x1[pp,scales_beta1] # that would be same scale accross sites at once
...
write_rds(jags_model, 
          file = glue("data/model_res/M06{spslist}_{park_name}1.rds"))
...
write_rds(samples_jags, 
          file = glue("data/model_res/M06{spslist}_{park_name}2.rds"))
