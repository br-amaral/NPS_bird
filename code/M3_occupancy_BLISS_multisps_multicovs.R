...
# -----------   occupancy_noscale   ----------
# ********************************************
# # 1 park, all scales by site specific, all sps
    ...
    for(j in 1:nsiteM){       # site j	
      for(t in 1:nyrM){       # year t	
        ...
        logit(psi[i,j,t]) <- beta0[i] + 	
                             beta1[i] * x1[j, scales_beta1[j]] # that would be same scale accross sites at once
...
write_rds(jags_model, 
          file = glue("data/model_res/m3_1.rds"))
...
write_rds(samples_jags, 
          ...
          file = glue("data/model_res/m3_2.rds"))
