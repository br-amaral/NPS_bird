...
y1 %>% select(AOU_Code, Admin_Unit_Code) %>% table() %>% sort()
    ...
    for(j in 1:nsiteM){       # site j	
      for(t in 1:nyrM){       # year t	
        ...
        logit(psi[i,j,t]) <- beta0[i] + 	
                             beta1[i] * x1[i, 5] # that would be same scale accross sites at once
                          
        # Observation model for replicated counts	
        for(k in 1:nintervalM){  # interval k
       
          logit(p[i,j,t,k]) <- alpha0[i] + alpha1[i] * day[i,j,t] + alpha2[i] * (day[i,j,t]^2) + alpha3[i] * time[i,j,t,k]
...
write_rds(jags_model, 
          file = glue("data/model_res/M01_1WBNU.rds"))
...
write_rds(samples_jags, 
          file = glue("data/model_res/M01_2WBNU.rds"))
