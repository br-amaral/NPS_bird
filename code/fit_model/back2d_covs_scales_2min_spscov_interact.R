...
script_name <- "back2d_covs_scales_2min_spscovs_interact.R"
...
jags_data <- list(
    ...
    scales_beta1 = loop_sca,
    scales_beta2 = loop_sca,
    scales_beta3 = loop_sca,
    scales_beta4 = loop_sca,
    scales_beta5 = loop_sca
)
...
for(loop_sca in 1:3){
  ...
  jags_data1$scales_beta1 <- loop_sca
  jags_data1$scales_beta2 <- loop_sca
  jags_data1$scales_beta3 <- loop_sca
  jags_data1$scales_beta4 <- loop_sca
  jags_data1$scales_beta5 <- loop_sca
  cat("\n\n\n running first jags \n\n\n\n")

  samples_jags <- jags(data = jags_data1,
                       inits = inits,
  ...
  cat("\n\n\n model done!!! \n\n\n\n")
  file_name <- glue("{sps_loop}_step{step_numb}_output_{date_step1}_allsca{loop_sca}_int")
  ...
  meta_name <- file(glue("data/ana_file/{sps_loop}_step{step_numb}_metadata_{date_step1}_allsca{loop_sca}_int.txt"))
