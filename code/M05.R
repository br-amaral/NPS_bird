...
y1 %>% select(AOU_Code) %>% table() %>% sort()

spslist <- "BTNW"

yf <- y1 %>% 
  filter(AOU_Code == spslist,
         Admin_Unit_Code == park_name
  ) 
...
# create 4d array
p <- which(pk == park_name, arr.ind = T)

yyy3 <- array(NA, dim = list(max(nsite_pk[p]), length(years), ninterval),
              dimnames = list(site_vec[1:max(nsite_pk[p])],
                              years,
                              seq(1,10,1)))
...
for(a in 1:nrow(yf3)){
  ...
  yyy3[j,t,k] <- ifelse(sum(yyy3[j,t,k],
                            yl %>% 
                              select(Bird_Count) %>% 
                              pull(), 
                            na.rm = T) > 0, 1, 0)
...
for(j in 1:dim(yyy3)[1]) {
  for(t in 1:dim(yyy3)[2]) {
    ind <- which.min(is.na(yyy3[j,t,])) %>% as.numeric()
    if(ind > 1) {
      yyy3[j,t,(1:(ind-1))] <- 0
    }
  }
...
time <- array(0, 
              dim = c(max(nsite_pk),
                      length(years), 
                      ninterval),
              dimnames = list(site_vec[1:max(nsite_pk)],
                              years,
                              seq(1,10,1)))
...
# forest area
arr_area <- read_rds(file = glue("data/out/arr_area_site.rds"))
...
arr_area1 <- arr_area[p,recent_dat,,] 
...
sink("models/mod_05.txt")	
cat("	
model {	
  # Community priors (with hyperparameters) for species-specific (i) parameters	

    beta0 ~ dnorm(0, 0.3676)	            # Abundance intercepts for each species and year	
  ...
  # Likelihood	
    for(j in 1:nsiteM){       # site j	
      for(t in 1:nyrM){       # year t	
        ...
        logit(psi[j,t]) <- beta0 + 	
                             beta1 * x1[j, scales_beta1] # that would be same scale accross sites at once
...
str(jags.data <- list(y = yyy3,                    # bird detection array
                      nsiteM = dim(yyy3)[1],       # number of sites
                      nyrM = dim(yyy3)[2],         # number of years
...
write_rds(jags_model, 
          file = glue("data/model_res/M05_1.rds"))
...
write_rds(samples_jags, 
          file = glue("data/model_res/M05_2.rds"))
