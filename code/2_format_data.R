...
# Data paths -------------------------------------------------------------------------------------
NPS_DATA_PATH <- file.path("data/src/NETNtib.rds")
...
if(for_int == "T") {
  field_dat1 <- field_dat1  %>% 
    filter(AOU_Code %in% spslist2$AOU_Code)       ## get only forest obligates
}
...
site_n_vec <- vector()
for(i in 1:npk) {
  site_n_vec <- c(site_n_vec, seq(1, nsite_pk[i], 1))
}

site_pk$site_n <- site_n_vec
