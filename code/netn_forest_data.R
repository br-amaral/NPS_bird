...
local_cors %>% 
  ...
  ggplot() +
  geom_boxplot(aes(x = park, y = value), fill="#BF87B3") +
  theme_bw() +
  facet_wrap(~metric, scales = "free_y") +
  theme(axis.text.x = element_text(angle = 90))

local_cors_p %>% 
  pivot_longer(cols = c(perfor_m, basare_m), names_to = 'metric', values_to = 'value') %>% 
  ggplot() +
  geom_point(aes(x = park, y = value, col= metric), size = 4) +
  theme_bw() +
  facet_wrap(~metric)
...
comp <- comp %>% 
  select(park, site, perfor, perfor_m, basare, basare_m)
